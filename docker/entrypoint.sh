...
# app/Pay is a persistent volume, so it otherwise hides payment code shipped
# in a newly built image. Overlay version-owned payment files on every deploy,
# while preserving merchant credentials and runtime logs created in the volume.
tar \
    --exclude='./*/Config/Config.php' \
    --exclude='./*/runtime.log' \
    --exclude='*.log' \
    -cf - \
    -C /usr/local/share/acg-faka/default-pay . \
    | tar -xf - -C /var/www/html/app/Pay
